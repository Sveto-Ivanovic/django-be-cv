<template>
    <n-form class="form-class embed-form" ref="formRef" label-placement="top" :label-width="80" :model="formValue"
        ...
        <div class="form-section">
            <h3 class="section-title">Embed into Pinecone</h3>
            ...
            <n-form-item label="Use Existing Index" path="index_name_selected" class="form-item indented-field">
                <n-select v-model:value="formValue.index_name_selected" placeholder="Select Index:"
                    :options="indexOptionsDense" />
            </n-form-item>

            <n-form-item label="Include Lexical Index" path="index_name_toggle" class="form-item">
                <n-radio-group v-model:value="formValue.index_name_toggle" name="radiogroup2" class="radio-group">
                    <n-space>
                        <n-radio value="false">Exclude Lexical Index</n-radio>
                        <n-radio value="true">Include Lexical Index</n-radio>
                    </n-space>
                </n-radio-group>
            </n-form-item>

            <n-form-item v-if="formValue.index_name_toggle == 'true'" label="Select Lexical Index:"
                path="index_name_lexical_selected" class="form-item indented-field">
                <n-select v-model:value="formValue.index_name_lexical_selected" placeholder="Select Lexical Index:"
...
const formValue = ref({
    ...
    index_name_toggle: 'false',
    index_name_lexical_selected: '',
    index_name_selected: '',
    data: '',
})
...
const rules: FormRules = {
    index_name_selected: [
        {
            validator: validateIndexNameSelected,
    ...
    index_name_lexical_selected: [
        {
            validator: validateLexicalIndexNameSeleceted,
            message: 'Please select index_name!',
            trigger: 'input'
        }
    ],
    embed_model: [
...
function validateIndexNameSelected(rule: FormItemRule, value: string): boolean {
    return formValue.value.index_name_selected !== ''
}

function validateLexicalIndexNameSeleceted(rule: FormItemRule, value: string): boolean {
    if (formValue.value.index_name_toggle == "true") {
        return formValue.value.index_name_lexical_selected != ''
    }
    else {
        return true
    }
...
const { mutateAsync: embedForm } = globalAPI.userPinecone.embedPineconeRecords(true)
const { mutateAsync: embedText } = globalAPI.userPinecone.embedPineconeRecords(false)
        ...
        if (!errors) {
            ...
            if (formValue.value.input_mode == 'text') {
                const data_text: PineconeEmbedRequestText = {
                    "index_name": formValue.value.index_name_selected,
                ...
                if (formValue.value.index_name_toggle == "true") {
                    data_text['lexical_index_name'] = formValue.value.index_name_lexical_selected
                }
                try {
                ...
                if (fileData.value && fileData.value.length > 0) {
                    ...
                    const data: PineconeEmbedRequestForm = {
                        "index_name": formValue.value.index_name_selected,
                        "embed_model": formValue.value.embed_model,
                        "input_mode": formValue.value.input_mode,
                    ...
                    if (formValue.value.index_name_toggle == "true") {
                        data['lexical_index_name'] = formValue.value.index_name_lexical_selected
                    }
                    if (formValue.value.chunk_config_toggle == "true") {
