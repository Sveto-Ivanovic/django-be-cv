...
async def fetch_db_memory(state: State):
    try:
        ...
        # If the chat history does not exist, create a new one
            ...
            print(f"New chat history created with ID: {state['conv_id']}")
            return state
...
async def check_limit_node(state: State):
    ...
    print(f"{state['conv_id']} - Checking limits: messages in history={len(state['chat_record_history'])}, query length={len(state['query'])}")
    try:
...
async def update_db_memory(state: State):
    ...
    try:
        ...
        if not chat_record_history:
            ...
            logger.info(f"Chat history id: {state['conv_id']}")
